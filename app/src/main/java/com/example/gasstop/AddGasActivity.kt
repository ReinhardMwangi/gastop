...
class AddGasActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        edtName = findViewById(R.id.mEdtGname)
                ...
                val uploadTask = ref?.putFile(filePath!!)!!.addOnCompleteListener {
                    ...
                    if (it.isSuccessful) {
                        ref.downloadUrl.addOnSuccessListener {
                            var gasData = Gasupload(name, price, it.toString(), id)
                            var ref = FirebaseDatabase.getInstance().getReference().child("Gas/$id")
                            ref.setValue(gasData)
                            Toast.makeText(this, "Gas submitted successfully", Toast.LENGTH_SHORT)
                                .show()
                        }
                    } else {
                        Toast.makeText(this, "Gas submission failed", Toast.LENGTH_SHORT).show()
                    }
                }
